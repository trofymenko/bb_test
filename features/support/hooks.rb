Before do |scenario|
  ...
  Howitzer::Cache.store(:cucumber, :scenario_title, scenario.name.gsub(/\s+/, '_'))
...
After do |scenario|
  if CapybaraHelpers.cloud_driver?
    ...
    Capybara.page.save_screenshot(
      "#{Howitzer::Cache.extract(:cucumber, :scenario_title)}-#{Time.now.strftime('%Y-%m-%d-%H:%M:%S')}.png"
    )
    # Capybara.page.save_page(
    #     "#{Howitzer::Cache.extract(:cucumber, :scenario_title)}-#{Time.now.strftime('%Y-%m-%d-%H:%M:%S')}.html"
    # )
  end
